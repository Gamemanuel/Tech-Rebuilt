...
import { ArchiveReceiptButton } from "@/components/archive-receipt-button";
...
export default async function ReceiptDetailPage({ params }: { params: { id: string } }) {
  ...
  const unassignedCount = assignable.filter((i) => i.unit_id === null).length;
  const fullyAssigned = assignable.length > 0 && unassignedCount === 0;
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              {receipt.archived_at && <Badge variant="secondary">Archived {formatDate(receipt.archived_at)}</Badge>}
              {!receipt.archived_at && fullyAssigned && <Badge variant="success">Fully assigned</Badge>}
              {!receipt.archived_at && !fullyAssigned && assignable.length > 0 && (
          ...
          {receipt.archived_at ? (
              ...
              <ArchiveReceiptButton receiptId={receipt.id} unassignedCount={unassignedCount} />
          )}
        </div>
